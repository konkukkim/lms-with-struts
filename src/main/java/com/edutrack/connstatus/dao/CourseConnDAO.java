 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class CourseConnDAO  extends AbstractDAO{
	private String connTable = "";

	/**
	 *
	 */
	public CourseConnDAO() {
		super();
	 ...
	 * @throws DAOException
	 */
	public ArrayList getCourseConCnt(String systemcode, CourseConnectDTO courseConnectDto) throws DAOException{
		/*
		 * courseConnectDto.getSelectGb()
		 * 		1	시간별
		 * 		2	일별
		 * 		3	월별
		 * 		4	년도별
		 ...
		 */
		 ...
		 sb.append(" from course_connect cc, course co ");
		 sb.append(" where cc.system_code=co.system_code and cc.course_id = co.course_id");
		 sb.append(" and cc.system_code = '"+systemcode+"' ");
		 sb.append(getPlusWhere(courseConnectDto));
		 connTable = "( "+sb.toString()+" ) ";

		return getCourseSelect(systemcode, courseConnectDto);
	}

	private String getPlusWhere(CourseConnectDTO courseConnectDto) throws DAOException{
		String retStr = "";
		 if(!courseConnectDto.getCourseId().equals(""))
		 	retStr +=" and co.course_id= '"+courseConnectDto.getCourseId()+"' ";
		return retStr;
	}

	public int getTotalCnt(String systemcode,  CourseConnectDTO courseConnectDto) throws DAOException{
		 StringBuffer sb = new StringBuffer();
		 QueryStatement sql = new QueryStatement();
      int retVal = 0;

       String startVal = getStartVal(courseConnectDto.getStartYear(),courseConnectDto.getStartMon(),courseConnectDto.getStartDay());
	    String endVal = getEndVal(courseConnectDto.getEndYear(),courseConnectDto.getEndMon(),courseConnectDto.getEndDay());
	...
	protected ArrayList getCourseSelect(String systemcode, CourseConnectDTO courseConnectDto) throws DAOException{
		 StringBuffer sb = new StringBuffer();
		 QueryStatement sql = new QueryStatement();
       ...
       String startVal = getStartVal(courseConnectDto.getStartYear(),courseConnectDto.getStartMon(),courseConnectDto.getStartDay());
	    ...
	    if(courseConnectDto.getSelectGb().equals("3"))
	     {
	   	...
	   	if(courseConnectDto.getSelectGb().equals("4"))
	     {
	   ...
	   if(courseConnectDto.getSelectGb().equals("5"))
	     {
	  ...
	  if(courseConnectDto.getSelectGb().equals("6"))
	     {
	  ...
	  if(courseConnectDto.getSelectGb().equals("7"))
	     {
	 ...
	 if(courseConnectDto.getSelectGb().equals("8"))
   {
       sb.append(" select co.so_name as get_time, co.code_so,  ifnull(count(su.user_id),0) as cnt  ");
   	 ...
   	 sb.append(" 	, case when (substring(u.post_code,1,3) = '417') then '11'/*인천(강화)*/  ");
		 ...
		 sql.setSql(sb.toString());
		 log.debug("[getCourseSelect]" + sql.toString());
	     String[] result = null;
		 RowSet rs = null;
		 int curCnt = 0;
		 try{
