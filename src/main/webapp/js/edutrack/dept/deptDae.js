	...
	function viewMessage() {
		var f = document.f;
	    var deptDaecode = f.pDeptDaecode.value;
		alert("'"+deptDaecode+"'는 사용중인 소속코드입니다.\n다른 소속코드를 입력해 주세요");
		f.pDeptDaecode.value ="";
		new Effect.Highlight("pDeptDaecode");
		f.pDeptDaecode.focus();
	}
	...
	function autoReloadCallback(data){
		...
		if(paggingStr != null && paggingStr !=''){
	  	...
	  	if(rowLength == 0){
		  	objStr += "<tr><td class=\"s_tab04_cen\" colspan=\"11\">등록된 시스템 코드가 없습니다.</td></tr>";
	  	}else{
		  	for(i=0;i<rowLength;i++){
				...
				objStr += "<tr onMouseOver=\"this.className='tab_over'\" onMouseOut=\"this.className='tab_out'\">"
					...
					+ "<a href=\"javascript:getDeptDaeInfo('U','"+deptDaecode+"')\">수정</a>/"
					+ "<a href=\"javascript:getDeptDaeInfo('D','"+deptDaecode+"')\">삭제</a>"
					+ "]</b></td>"
					+ "</tr>";
	...
	function manageDeptDae(mode){
	   if(mode == "Add"){		// 등록시
		    ...
		    if(mode == "Delete" && deptSoCnt > 0){		// 삭제시 하위소코드 있는지 여부 체크
		    	alert("등록된 소코드가 있습니다.\n소코드를 먼저 삭제 하세요!!!");
		    	return;
		    }else{
		   	   if(formCheck()){
			       ...
			       if(mode == "Edit")
			       	   ...
			       	   flagStr = "소속코드를 삭제 할 경우 시스템 전반에 영향을 미칠 수 있습니다.\n삭제 하시겠습니까?";
