...
<%
	...
	if(curriSubInfo.getPrice() > 0){
		...
		tmpService = "결제일 부터 ";
		EnrollLink = CONTEXTPATH+"/Student.cmd?cmd=enrollRegist";
	}else{
		tmpPrice = "무료";
		tmpService = "신청일 부터 ";
		EnrollLink = CONTEXTPATH+"/Student.cmd?cmd=enrollProcess";
	}
...
function makeWin() {
	...
	if (frm.paymethod.value=='Free') {
		frm.action ="<%=EnrollLink%>";
		frm.submit();
	}
	else {
...
<input type="hidden" name="amt" value="<%=curriSubInfo.getPrice()%>">
...
<input type="hidden" name="pCurriType" value="<%=curriType%>">
            ...
            <table width="100%" height="35" border="0" cellpadding="0" cellspacing="0" background="<%=CONTEXTPATH%>/img/<%=SYSTEMCODE%>/courses/02_cen_titlebg.gif">
              <tr> 
                <td align="left" valign="middle" style="padding:0 0 0 8"><img src="<%=CONTEXTPATH%>/img/<%=SYSTEMCODE%>/courses/02_centitle05.gif" width="134" height="19"></td>
              </tr>
            </table>
...
<%=tmpService%>  <%=StringUtil.nvl(curriSubInfo.getServiceStart())%> 일간
<%
	}
%>
								  </td>
...
%>
                  ...
                  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr> 
                      <td height="20" align="left" valign="top"  style="padding:0 0 0 0">
...
핸드폰결제-->
                    ...
                    <tr> 
                      <td align="left" valign="top"  style="padding:5 0 0 0"><table width="50%" border="0" cellpadding="0" cellspacing="2" bgcolor="#C9CDD3">
                          <tr> 
...
<input type="text" name="paymethodname" value="신용카드" style="font-weight:bold; color:red; text-align:center; border-width:1px; border-color:white; border-style:solid;" readonly>
                    ...
                    <tr> 
                      <td align="left" valign="top" style="padding:15 0 20 0"><table width="100%" border="0" cellpadding="15" cellspacing="1" bgcolor="#C9CDD3">
                          <tr> 
                            <td bgcolor="#FFFFFF"><strong><font color="#333333">[ 
                              ...
                              내용은 해당 카드사를 참조하시기 바랍니다.<br> 
							  <img src="<%=CONTEXTPATH%>/img/<%=SYSTEMCODE%>/common/blank.gif" width="15" height="15" align="absmiddle">
							  <a href="http://card.kbstar.com/quics?page=s_crd" target="_blank" class="b_title02">국민카드</a><img src="<%=CONTEXTPATH%>/img/<%=SYSTEMCODE%>/common/blank.gif" width="15" height="15" align="absmiddle"> 
                              <a href="http://www.bccard.com" target="_blank" class="b_title02">BC카드</a><img src="<%=CONTEXTPATH%>/img/<%=SYSTEMCODE%>/common/blank.gif" width="15" height="15" align="absmiddle"> 
                              <a href="http://www.shinhancard.com" target="_blank" class="b_title02">조흥카드</a><img src="<%=CONTEXTPATH%>/img/<%=SYSTEMCODE%>/common/blank.gif" width="15" height="15" align="absmiddle"> 
                              <a href="http://www.wooricard.com" target="_blank" class="b_title02">우리카드<br>
                        ...
                        <table width="100%" border="0" cellpadding="15" cellspacing="1" bgcolor="#C9CDD3">
                          <tr> 
                            <td bgcolor="#FFFFFF"><strong><font color="#333333">[ 
                              ...
                              <br>
                              <font color="#0066CC">SK텔레콤</font> : 10만원(High고객), 
                              6만원(Medium고객), 3만원(Low고객)<br>
                              가. 결제 건당 한도금액 : 10만원<br>
                              나. 단, 신규가입 고객은 Low고객으로 3만원 한도<br>
                              <br>
                              <font color="#0066CC">KTF</font> : 6만원 (모든 고객)<br>
                              <br>
                              <font color="#0066CC">LG텔레콤</font> : 10만원 (모든 고객) 
                            </td>
                          </tr>
                        </table>
                        <table width="100%" border="0" cellpadding="15" cellspacing="1" bgcolor="#C9CDD3">
                          <tr> 
                            <td bgcolor="#FFFFFF"><strong><font color="#333333">[ 
                              ...
                              1. 인터넷으로 상품구매와 동시에 본인 통장에서 한국디자인진흥원으로 바로 입금처리가 
                              됩니다.<br>
                              2. 실시간 계좌이체시 은행별 서비스 가능시간 을 확인바랍니다.<br>
                              3. 고객님의 실시간 계좌이체 내역 을 확인할 수 있습니다.<br>
                              4. 실시간 계좌이체로 주문처리 중 병행 결제한 카드의 장애시 발생할 경우는 이체 처리 
                              후 취소처리되어<br>
                              <img src="<%=CONTEXTPATH%>/img/<%=SYSTEMCODE%>/common/blank.gif" width="15" height="15" align="absmiddle">바로 
                              고객님의 통장에 재입금 됩니다.<br>
                              5. 실시간 계좌이체는 10분 이내로 입금확인을 하실 수 있습니다.<br>
                              6. 송금 수수료는 한국디자인진흥원에서 부담합니다.</td>
                          </tr>
                        </table>
                        <table width="100%" border="0" cellpadding="15" cellspacing="1" bgcolor="#C9CDD3">
                          <tr> 
