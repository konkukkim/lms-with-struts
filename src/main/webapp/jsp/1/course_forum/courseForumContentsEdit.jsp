...
<script language="javascript">
	...
	function goDel(){
		/*
		var f = document.Input;
		f.action="<%=CONTEXTPATH%>/CourseForumContents.cmd?cmd=courseForumContentsDelete&pCourseId=<%=pCourseId%>&pForumId=<%=pForumId%>&pSeqNo=<%=contentsDto.getSeqNo()%>";
...
<!-- form start ----------------->
<form name=Input method="post" action="<%=CONTEXTPATH%>/CourseForumContents.cmd?cmd=courseForumContentsRegist&pRegMode=Edit&pCourseId=<%=pCourseId%>&pForumId=<%=pForumId%>&pSeqNo=<%=contentsDto.getSeqNo()%>&pRegDate=<%=contentsDto.getRegDate()%>" enctype="multipart/form-data" onSubmit="return submitCheck()">
