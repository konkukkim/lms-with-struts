...
//		if (!checkBoxSelected() ) return ;
	...
	function deleteManifest(manifest_id) {
		var chkDel = confirm('등록된 교재 정보를 삭제 하시겠습니까?');
		if(chkDel){
			var keyWord = medilms.pKeyWord.value;
			medilms.method = "post";
			medilms.action = "<%=CONTEXTPATH%>/LCMS/deletemanifest.jsp?manifest="+manifest_id;
			medilms.target = "hiddenFrame";
			medilms.flag.value = "Q";
			medilms.submit() ;	
		}
	...
	function main(arg) {
		...
		switch(arg) {
			case 'I' :
				...
				medilms.method = "post";
				medilms.action = "<%=CONTEXTPATH%>/Contents.cmd?cmd=SiCMRegist";
				//medilms.target=self;
				medilms.submit();
				break;			
			case 'X' :
		...
		<td><!-- inner table setart -->
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				<tr align="center" bgcolor="#FFFFFF">
					<td height="20">* 대분류명을 클릭 하시면 해당 교재 정보를 삭제 하실 수 있습니다.</td>
				</tr>
				...
				<tr align="center" >
					<td background="<%=CONTEXTPATH%>/img/<%=SYSTEMCODE%>/SiCM/shadow.gif" ><img src="<%=CONTEXTPATH%>/img/<%=SYSTEMCODE%>/SiCM/shadow.gif" width="2" height="3"></td>
				</tr>
				<tr height='25'>
					<td align=center>Manifest 검색 <input type=text name='pKeyWord' value='<%=pKeyWord%>'> <input type=button value='검색' onClick='searchManifest()'>
					* 검색시 대/소문자 구분합니다.</td>
				</tr>
				<tr align="center" >
					<td background="<%=CONTEXTPATH%>/img/<%=SYSTEMCODE%>/SiCM/shadow.gif" ><img src="<%=CONTEXTPATH%>/img/<%=SYSTEMCODE%>/SiCM/shadow.gif" width="2" height="3"></td>
