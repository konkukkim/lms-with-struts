...
<form name=Input method="post" action="<%=CONTEXTPATH%>/PopMessage.cmd?cmd=messageInsert" enctype="multipart/form-data">
<%
	if(pMessageKb.equals("S")){
		for(int i=0;i < pTotCnt;i++){
			usersDto = (UsersDTO)chkList.get(i);
%>
			<input type="hidden" name="pCHK" value="<%=usersDto.getUserId()+"^"+usersDto.getUserName()%>">
<%
		}
	}
%>
...
<input type="hidden" name="pResultCode3"        value="<%=pResultCode3      %>">
	<table width="100%" height="100%" border="0">
		<tr>
			...
			<td class="pop_topbg"><img src="<%=CONTEXTPATH%>/img/<%=SYSTEMCODE%>/common/blet05.gif" align="absmiddle"><font class="pop_tit">쪽지쓰기</font></td>
...
<script language=javascript>Button3("확인", "submitCheck()", "");</script>
		...
		<tr>
			...
			<a href="javascript:self.close();"><img src="<%=CONTEXTPATH%>/img/<%=SYSTEMCODE%>/button_img/btn_popclose.gif"></a></td>
		</tr>
	</table>
