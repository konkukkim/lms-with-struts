...
<script language="javascript">
	...
	function goView(seq_no, send_id)
	{
        var f = document.f;
        var curPage = f.curPage.value;
		document.location = "<%=CONTEXTPATH%>/Message.cmd?cmd=messageShow&pSEQ_NO="+seq_no+"&pSEND_ID="+send_id+"&curPage="+curPage;
	}
...
<script language=javascript>Button5("받은쪽지함", "goReceiveMessage()", "");</script>
															</td>
														</tr>
													</table>
												</td>
...
<%
	ListDTO listObj = (ListDTO)model.get("sendMessageList");
%>
<%= listObj.getPageScript("f", "curPage", "goPage") %>
<!-- form start -->
<form name="f" method="post" action="<%=CONTEXTPATH%>/Message.cmd?cmd=sendMessagePagingList" >
...
<input type="hidden" name="checkYn" value="N">
											...
											<tr class="s_tab02">
												<td width="40"><a href="javascript:setAllToken(f.pCHK)">선택</a></td>
	...
	if(listObj.getItemCount() > 0){
		...
		while(list.next()){
			...
			if(!StringUtil.nvl(list.getString("receive_date")).equals("")){
				receive_date = DateTimeUtil.getDateType(6, StringUtil.nvl(list.getString("receive_date")));
			}else{
				receive_date = "읽지않음";
			}
%>
											<tr onMouseOver="this.className='tab_over'" onMouseOut="this.className='tab_out'">
												...
												<td></td>
												<td class="s_tab04"><a href="javascript:goView('<%=list.getInt("seq_no")%>','<%=StringUtil.nvl(list.getString("sender_id"))%>')" title="<%=StringUtil.nvl(list.getString("subject"))%>"><%=StringUtil.setMaxLength(StringUtil.nvl(list.getString("subject")),30)%></a></td>
												<td></td>
...
%>
												...
												<td></td>
												<td class="s_tab04_cen"><%=StringUtil.nvl(list.getString("receiver_name"))%></td>
												<td></td>
												<td class="s_tab04_cen"><%=DateTimeUtil.getDateType(1, StringUtil.nvl(list.getString("send_date")))%></td>
												<td></td>
												<td class="s_tab04_cen"><%=receive_date%></td>
											</tr>
<%			if(No > 1) { %>
											<tr class="s_tab03">
												<td colspan="11"></td>
...
%>
											<tr>
												<td class="s_tab04_cen" colspan="11">보낸쪽지가 없습니다.</td>
											</tr>
<%
	}
	list.close();
