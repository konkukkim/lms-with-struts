...
<%@ page import ="javax.sql.RowSet,com.edutrack.common.DateSetter,com.edutrack.common.dto.DateParam"%>
<%@ page import ="com.edutrack.common.dto.CodeParam,com.edutrack.common.dto.EditorParam"%>
...
<%
    ...
    String titleImg = "";
    if (pBbsType.equals("B") ) {
        titleImg = img_path+ "/community/tit_07.gif";
    } else if( pBbsType.equals("F") ) {
        titleImg = img_path+ "/community/tit_06.gif";
    } else if( pBbsType.equals("N") ) {
        titleImg = img_path+ "/community/tit_08.gif";
    }
%>
<center>
...
//		}
	...
	function goDel(){
		document.location.href="<%=CONTEXTPATH%>/CommSubBoard.cmd?cmd=bbsContentsDelete&pMode=<%=pMode%>&pCommId=<%=pCommId%>&pBbsId=<%=pBbsId%>&pSeqNo=<%=contentsDto.getSeqNo()%>&pBbsType=<%=pBbsType%>";
	}

	function delFile(fileNum){
       	document.location.href="<%=CONTEXTPATH%>/CommSubBoard.cmd?cmd=bbsContentsFileDelete&pMode=<%=pMode%>&pCommId=<%=pCommId%>&pBbsId=<%=pBbsId%>&pBbsType=<%=pBbsType%>&pSeqNo=<%=contentsDto.getSeqNo()%>&pFileNo="+fileNum;
    }

    function goList(){
		document.location.href="<%=CONTEXTPATH%>/CommSubBoard.cmd?cmd=commSubBoard&MENUNO=0&pBbsId=<%=pBbsId%>&pBbsType=<%=pBbsType%>";
	}
...
<form name=Input method="post" action="<%=CONTEXTPATH%>/CommSubBoard.cmd?cmd=commSubBoardRegist&pBbsType=<%=pBbsType%>&pRegMode=Edit&pCommId=<%=pCommId%>&pBbsId=<%=pBbsId%>&pSeqNo=<%=contentsDto.getSeqNo()%>&pRegDate=<%=contentsDto.getRegDate()%>" enctype="multipart/form-data">
...
%>
													<input type=file name="pFile[<%=i%>]" size=70 class=input_member>
												</td>
											</tr>
											<tr class="com_tab03">
												<td colspan="2"></td>
